 ...
 *
 * This software should run on node1.
 *
...
//#define	ENABLE_PATH1	// node0 to node1
//#define	ENABLE_PATH2	// node1 to node0
...
#define	ENABLE_PATH6	// node2 to node1
	...
	OS_STK    node1_sendMSG_to_node0_task_stk[TASK_STACKSIZE];
	OS_STK	  node1_recvMSG_from_node0_task_stk[TASK_STACKSIZE];
	OS_STK    node1_sendMSG_to_node2_task_stk[TASK_STACKSIZE];
	OS_STK	  node1_recvMSG_from_node2_task_stk[TASK_STACKSIZE];
	...
	#define	NODE1_RECVMSG_FROM_NODE0_TASK_PRIO	21
	#define	NODE1_RECVMSG_FROM_NODE2_TASK_PRIO	22
	#define NODE1_SENDMSG_TO_NODE0_TASK_PRIO	23
	#define NODE1_SENDMSG_TO_NODE2_TASK_PRIO	24
#endif

#ifdef LINUX
/* uClinux specific task definitions */
	pthread_t node1_sendMSG_to_node0_thread;
	pthread_t node1_sendMSG_to_node2_thread;
	pthread_t node1_recvMSG_from_node0_thread;
	pthread_t node1_recvMSG_from_node2_thread;
...
// alive.
	int node1_sendMSG_to_node0_thread_flag = 0;
	int node1_sendMSG_to_node2_thread_flag = 0;
	int node1_recvMSG_from_node0_thread_flag = 0;
	int node1_recvMSG_from_node2_thread_flag = 0;
...
void node1_recvMSG_from_node0_task(void* pdata)
{
	...
	for(cycles = 0; cycles < NUM_OF_CYCLES; cycles++) {
		...
		for(msg_size = 1; msg_size < MSG_SIZE_MAX; msg_size++) {
			if(ISNONBLOCKING) {
				mcapi_msg_recv_i(node1_recvEP_from_node0, msg, msg_size, &r1, &status);
				...
				mcapi_msg_recv(node1_recvEP_from_node0, msg, msg_size, &tSize, &status);
				if(status != MCAPI_SUCCESS) {
					check_status(status);
				}
			}
			...
			for(i = 0; i < msg_size; i++) {
				if(msg[i] != (i % 256)) {  // 0 <= msg[i] <= 255
					msg_err++;
					printf("node1_recvMSG_from_node0_task: receive error on receiving message with size %d from node0\n", msg_size);
					printf("node1_recvMSG_from_node0_task:    - expected: 0x%x, received: 0x%x\n", i, msg[i]);
				}
			}

//			printf("node1_recvMSG_from_node0_task: message with %d bytes received from node0\n", msg_size); fflush(stdout);
		}
		printf("node1_recvMSG_from_node0_task: %d super-cycles executed\n", cycles);
	}
	printf("node1_recvMSG_from_node0_task: %d messages (%d bytes) received from node0 with %d errors\n", msg_cnt, byte_cnt, msg_err);

	// delete local endpoints
	mcapi_endpoint_delete(node1_recvEP_from_node0, &status);
	check_status(status);
//	printf("node1_recvMSG_from_node0_task: Receive endpoint node1_recvEP_from_node0 deleted\n");

	node1_recvMSG_from_node0_thread_flag = 0;
	printf("node1_recvMSG_from_node0_task will finish in 1 second\n"); fflush(stdout);
	usleep(1000000);
...
/**************************************************************
 * task: node1_recvMSG_from_node2_task()
 * Task will receive messages from node2 and will check
 * message contents for correctness.
 *************************************************************/
void node1_recvMSG_from_node2_task(void* pdata)
{
	mcapi_timeout_t timeout = MCA_INFINITE;
	mcapi_status_t status = MCAPI_TRUE;
	unsigned char msg[MSG_SIZE_MAX];
	...
	for(cycles = 0; cycles < NUM_OF_CYCLES; cycles++) {
		...
		for(msg_size = 1; msg_size < MSG_SIZE_MAX; msg_size++) {
			if(ISNONBLOCKING) {
				mcapi_msg_recv_i(node1_recvEP_from_node2, msg, msg_size, &r1, &status);
				mcapi_wait(&r1, &tSize, timeout, &status);
				if(status != MCAPI_SUCCESS) {
					check_status(status);
					if(status == MCAPI_TIMEOUT)	break;
				...
				mcapi_msg_recv(node1_recvEP_from_node2, msg, msg_size, &tSize, &status);
				if(status != MCAPI_SUCCESS) {
					check_status(status);
				}
			}
			...
			for(i = 0; i < msg_size; i++) {
				if(msg[i] != (i % 256)) {  // 0 <= msg[i] <= 255
					msg_err++;
					printf("node1_recvMSG_from_node2_task: receive error on receiving message with size %d from node2\n", msg_size);
					printf("node1_recvMSG_from_node2_task:    - expected: 0x%x, received: 0x%x\n", i, msg[i]);
				}
			}

//			printf("node1_recvMSG_from_node2_task: message with %d bytes received from node2\n", msg_size); fflush(stdout);
		}
		printf("node1_recvMSG_from_node2_task: %d super-cycles executed\n", cycles);
	}
	printf("node1_recvMSG_from_node2_task: %d messages (%d bytes) received from node2 with %d errors\n", msg_cnt, byte_cnt, msg_err);

	// delete local endpoints
	mcapi_endpoint_delete(node1_recvEP_from_node2, &status);
	check_status(status);
//	printf("node1_recvMSG_from_node2_task: Receive endpoint node1_recvEP_from_node2 deleted\n");

	node1_recvMSG_from_node2_thread_flag = 0;
	printf("node1_recvMSG_from_node2_task will finish in 1 second\n"); fflush(stdout);
	usleep(1000000);
...
/**************************************************************
 * task: node1_sendMSG_to_node0_task()
 * Sends messages with different message sizes to node1.
 **************************************************************/
void node1_sendMSG_to_node0_task(void* pdata)
{
	mcapi_status_t status = MCAPI_TRUE;
...
//				if(msg_size == 27) msg[13] = 0xfe;	// insert an error
			...
			if(ISNONBLOCKING) {
				...
				mcapi_msg_send_i(node1_sendEP_to_node0, node0_recvEP_from_node1, msg, msg_size, prio, &r1, &status);
				...
				mcapi_msg_send(node1_sendEP_to_node0, node0_recvEP_from_node1, msg, msg_size, prio, &status);
				if(status != MCAPI_SUCCESS)
					printf("node1_sendMSG_to_node0_task: Error in mcapi_msg_send() with msg_size = %d\n", msg_size); fflush(stdout);
					check_status(status);
			}
...
//			printf("node1_sendMSG_to_node0_task: Message with %d bytes sent to node1\n", msg_size);
		}
		printf("node1_sendMSG_to_node0_task: %d super-cycles executed\n", cycles);
	}
	printf("node1_sendMSG_to_node0_task: %d messages (%d bytes) sent to node0\n", msg_cnt, byte_cnt);

	// delete local endpoints
	mcapi_endpoint_delete(node1_sendEP_to_node0, &status);
	check_status(status);
//	printf("node1_sendMSG_to_node0_task: Send endpoint node1_sendEP_to_node0 deleted\n");

	node1_sendMSG_to_node0_thread_flag = 0;
	printf("node1_sendMSG_to_node0_task will finish in 1 second\n"); fflush(stdout);
	usleep(1000000);
...
/**************************************************************
 * task: node1_sendMSG_to_node2_task()
 * Sends messages with different message sizes to node1.
 **************************************************************/
void node1_sendMSG_to_node2_task(void* pdata)
{
	mcapi_status_t status = MCAPI_TRUE;
	char msg[MSG_SIZE_MAX];	// message buffer
	int	msg_size;			// current message size
...
//				if(msg_size == 27) msg[13] = 0xfe;	// insert an error
			...
			if(ISNONBLOCKING) {
				// non-blocking send method will be used
				mcapi_msg_send_i(node1_sendEP_to_node2, node2_recvEP_from_node1, msg, msg_size, prio, &r1, &status);
				while(!(mcapi_test(&r1, &tSize1, &status))) {
					if(status != MCAPI_SUCCESS)
						check_status(status);
					OSTimeDly(1);
				...
				// blocking send method will be used
				mcapi_msg_send(node1_sendEP_to_node2, node2_recvEP_from_node1, msg, msg_size, prio, &status);
				if(status != MCAPI_SUCCESS) {
					printf("node1_sendMSG_to_node2_task: Error in mcapi_msg_send() with msg_size = %d\n", msg_size); fflush(stdout);
					check_status(status);
				}
			}
...
//			printf("node1_sendMSG_to_node2_task: Message with %d bytes sent to node2\n", msg_size);
		}
//		printf("node1_sendMSG_to_node2_task: %d super-cycles executed\n", cycles);
	}
	printf("node1_sendMSG_to_node2_task: %d messages (%d bytes) sent to node2\n", msg_cnt, byte_cnt);

	// delete local endpoints
	mcapi_endpoint_delete(node1_sendEP_to_node2, &status);
	check_status(status);
//	printf("node1_sendMSG_to_node2_task: Send endpoint node1_sendEP_to_node2 deleted\n");

	node1_sendMSG_to_node2_thread_flag = 0;
	printf("node1_sendMSG_to_node2_task will finish in 1 second!\n"); fflush(stdout);
	usleep(1000000);
...
void init_task(void* pdata)
{
	...
	/* Initialize MCAPI **************************************/
	// MY_DOMAIN and NIOS_1_NODE_ID are defined in file mappings.h
	mcapi_initialize(MY_DOMAIN, NIOS_1_NODE_ID, NULL, &mcapi_parameters, &mcapi_info, &status);
	check_status(status);
...
/* PATH1 related initialization **********************************/
#ifdef	ENABLE_PATH1
	// create local receive endpoint
	node1_recvEP_from_node0 = mcapi_endpoint_create(NODE1_RECVPORT_FROM_NODE0, &status);
	check_status(status);
	printf("init_task:   - Local endpoint on node1 (used to receive data from node0) created\n"); fflush(stdout);

	// create receive task receiving data from node0
#ifdef UCOSII
	err = OSTaskCreateExt(node1_recvMSG_from_node0_task,
						  NULL,
						  (void *)&node1_recvMSG_from_node0_task_stk[TASK_STACKSIZE-1],
						  NODE1_RECVMSG_FROM_NODE0_TASK_PRIO,
						  NODE1_RECVMSG_FROM_NODE0_TASK_PRIO,
						  node1_recvMSG_from_node0_task_stk,
						  TASK_STACKSIZE,
						  NULL,
						  0);
	if(err != OS_NO_ERR) {
	...
	err = pthread_create(&node1_recvMSG_from_node0_thread, NULL, (void*)&node1_recvMSG_from_node0_task, NULL);
	if(err != 0) {
		printf("init_task: Error in pthread_create\n");
	}
#endif // LINUX
	node1_recvMSG_from_node0_thread_flag = 1; 	// mark thread as alive
#endif // PATH1

/* PATH6 related initialization **********************************/
#ifdef	ENABLE_PATH6
	// create local receive endpoint
	node1_recvEP_from_node2 = mcapi_endpoint_create(NODE1_RECVPORT_FROM_NODE2, &status);
	check_status(status);
	printf("init_task:   - Local endpoint on node1 (used to receive data from node2) created\n"); fflush(stdout);

	// create receive task receiving data from node2
#ifdef UCOSII
	/* uC/OS-II specific code */
	err = OSTaskCreateExt(node1_recvMSG_from_node2_task,
						  NULL,
						  (void *)&node1_recvMSG_from_node2_task_stk[TASK_STACKSIZE-1],
						  NODE1_RECVMSG_FROM_NODE2_TASK_PRIO,
						  NODE1_RECVMSG_FROM_NODE2_TASK_PRIO,
						  node1_recvMSG_from_node2_task_stk,
						  TASK_STACKSIZE,
						  NULL,
						  0);
	if(err != OS_NO_ERR) {
...
#ifdef LINUX
/* uClinux specific code */
	err = pthread_create(&node1_recvMSG_from_node2_thread, NULL, (void*)&node1_recvMSG_from_node2_task, NULL);
	if(err != 0) {
		printf("init_task: Error in pthread_create\n");
	}
#endif // LINUX
	node1_recvMSG_from_node2_thread_flag = 1;	// mark thread as alive
#endif // PATH6

/* PATH2 related initialization **********************************/
#ifdef	ENABLE_PATH2
	// create local send endpoint
	node1_sendEP_to_node0 = mcapi_endpoint_create(NODE1_SENDPORT_TO_NODE0, &status);
	check_status(status);
	printf("init_task:   - Local endpoint on node1 (used to send data to node0) created\n"); fflush(stdout);

	/* get remote receive endpoints **********************************/
	node0_recvEP_from_node1 = mcapi_endpoint_get(MY_DOMAIN, NIOS_0_NODE_ID, NODE0_RECVPORT_FROM_NODE1, MCAPI_TIMEOUT_INFINITE, &status);
	check_status(status);
	printf("init_task:   - Remote endpoint on node0 (used to receive data from node1) received\n"); fflush(stdout);

	// create send task sending data to node0
#ifdef UCOSII
	/* uC/OS-II specific code */
	err = OSTaskCreateExt(node1_sendMSG_to_node0_task,
						  NULL,
						  (void *)&node1_sendMSG_to_node0_task_stk[TASK_STACKSIZE-1],
						  NODE1_SENDMSG_TO_NODE0_TASK_PRIO,
						  NODE1_SENDMSG_TO_NODE0_TASK_PRIO,
						  node1_sendMSG_to_node0_task_stk,
						  TASK_STACKSIZE,
						  NULL,
						  0);
	if(err != OS_NO_ERR) {
...
#ifdef LINUX
	/* uClinux specific code */
	err = pthread_create(&node1_sendMSG_to_node0_thread, NULL, (void*)&node1_sendMSG_to_node0_task, NULL);
	if(err != 0) {
		printf("init_task: Error in pthread_create\n");
	}
#endif // LINUX
	node1_sendMSG_to_node0_thread_flag = 1;	// mark thread as alive
#endif //PATH2

/* PATH5 related initialization **********************************/
#ifdef	ENABLE_PATH5
	// create local send endpoint
	node1_sendEP_to_node2 = mcapi_endpoint_create(NODE1_SENDPORT_TO_NODE2, &status);
	check_status(status);
	printf("init_task:   - Local endpoint on node1 (used to send data to node2) created\n"); fflush(stdout);

	/* get remote receive endpoints **********************************/
	node2_recvEP_from_node1 = mcapi_endpoint_get(MY_DOMAIN, NIOS_2_NODE_ID, NODE2_RECVPORT_FROM_NODE1, MCAPI_TIMEOUT_INFINITE, &status);
	check_status(status);
	printf("init_task:   - Remote endpoint on node2 (used to receive data from node1) received\n"); fflush(stdout);

	// create send task sending data to node2
#ifdef UCOSII
	/* uC/OS-II specific code */
	err = OSTaskCreateExt(node1_sendMSG_to_node2_task,
						  NULL,
						  (void *)&node1_sendMSG_to_node2_task_stk[TASK_STACKSIZE-1],
						  NODE1_SENDMSG_TO_NODE2_TASK_PRIO,
						  NODE1_SENDMSG_TO_NODE2_TASK_PRIO,
						  node1_sendMSG_to_node2_task_stk,
						  TASK_STACKSIZE,
						  NULL,
						  0);
	if(err != OS_NO_ERR) {
...
#ifdef LINUX
	/* uClinux specific code */
	err = pthread_create(&node1_sendMSG_to_node2_thread, NULL, (void*)&node1_sendMSG_to_node2_task, NULL);
	if(err != 0) {
		printf("init_task: Error in pthread_create\n");
	}
#endif // LINUX
	node1_sendMSG_to_node2_thread_flag = 1;	// mark thread as alive
#endif // PATH5

	// wait till the created threads have been finished.
	while((node1_sendMSG_to_node0_thread_flag +
		   node1_sendMSG_to_node2_thread_flag +
		   node1_recvMSG_from_node0_thread_flag +
		   node1_recvMSG_from_node2_thread_flag) != 0)
		usleep(1000000);
